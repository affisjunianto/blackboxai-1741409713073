...
if(isset($_GET['agent_token']) && isset($_GET['agent_code'])){
    ...
    if(empty($agent_token) || empty($agent_code)){
        ...
        if($data_op['agent_status'] == "blocked"){
            ...
            $cek_p = mysqli_query($call, "SELECT * FROM tb_provider");
            while($data_p = mysqli_fetch_array($cek_p)){
                $data[] = array(
                    'provider_code' => $data_p['provider_code'],
                    'provider_name' => $data_p['provider_name'],
                    'provider_type' => $data_p['provider_type'],
                    'provider_image' => $data_p['provider_image'],
                    'provider_status' => $data_p['provider_status']);
            }
            $res = array('status' => 'success', 'provider' => $data, 'msg' => 'SUCCESS');
        }
    }
} else {
    $res = array('status' => 'fail', 'msg' => 'Invalid Parameter');
